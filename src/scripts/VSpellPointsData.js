...
export class VSpellPointsData {
	...
	static initPoints(actor) {
		...
		let defaultResources = foundry.utils.mergeObject(this.resourcesTemplate, actorResources, {
			insertKeys: false,
			overwrite: true,
			recursive: true,
		});
	...
	static moduleManuallyEnabled(actor) {
		...
		switch (actorSetting) {
			case undefined:
				return undefined;
			case VSpellPoints.STATUSCHOICES.enabled:
				return true;
			case VSpellPoints.STATUSCHOICES.disabled:
				return false;
			case VSpellPoints.STATUSCHOICES.default:
				return undefined;
			default:
				return undefined;
		}
